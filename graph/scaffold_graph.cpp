...
static bool Compare(const ScaffoldGraphEdgeAdaptor &x, const ScaffoldGraphEdgeAdaptor &y) {
    ...
    for (unsigned i = 0; i < vertices_.size(); ++i) {
        for (int strand = 0; strand < 2; ++strand) {
            ...
            for (unsigned j = 0; j < edges.size(); ++j) {
                if (edges[j].status().IsDead() || edges[j].from().status().IsDead() ||
    ...
    for (unsigned i = 0; i < vertices_.size(); ++i) {
        for (int strand = 0; strand < 2; ++strand) {
            ...
            for (unsigned j = 0; j < edges.size(); ++j) {
                ...
                edges[j].Parse();
                double expected = ExpectedEdges(edges[j].level(), edges[j].from().path().size(),
                                                edges[j].to().path().size(), edges[j].distance());

                int level = edges[j].level();
                if (is_uneven && edges[j].from().path().size() > mean_[level] * 4 &&
                    edges[j].to().path().size() > mean_[level] * 4) {
                    double tmp = ExpectedEdges(
                        edges[j].level(), edges[j].from().path().size(), edges[j].to().path().size(),
                        edges[j].distance(),
    ...
    for (unsigned i = 0; i < vertices_.size(); ++i) {
        for (int strand = 0; strand < 2; ++strand) {
            ...
            for (unsigned j = 0; j < edges.size(); ++j) {
                ...
                if ((int)edges[j].values().size() < min_pairs || (int)edges[j].from().path_size() < min_length ||
