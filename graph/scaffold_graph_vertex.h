...
class ScaffoldGraphVertex {
  public:
    explicit ScaffoldGraphVertex(const ContigGraphPath &path = ContigGraphPath()) : path_(path) {}

    ScaffoldGraphVertex(const ScaffoldGraphVertex &x) : path_(x.path_), id_(x.id_), status_(x.status_) {}
...
class ScaffoldGraphVertexAdaptor {
  public:
    ...
    const ScaffoldGraphVertexAdaptor &ReverseComplement() {
    ...
    //    const uint32_t &in_kmer_count() const { return !is_reverse_ ? vertex_->in_kmer_count() :
    //    vertex_->out_kmer_count(); }
    //
    //    uint32_t &out_kmer_count() { return !is_reverse_ ? vertex_->out_kmer_count() : vertex_->out_kmer_count(); }
    //    const uint32_t &out_kmer_count() const { return !is_reverse_ ? vertex_->out_kmer_count() :
    //    vertex_->out_kmer_count(); }

    //    SequenceCount counts()
    //    {
    //        if (!is_reverse_) return vertex_->counts();
    //        else { SequenceCount counts = vertex_->counts(); std::reverse(counts.begin(), counts.end()); return
    //        counts; }
    //    }
    //
    ...
    //    { return (!is_reverse_) ? vertex_->get_count(index) : vertex_->get_count(vertex_->counts().size() - 1 -
    //    index); }
    //
