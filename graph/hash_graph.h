...
// Enforce hash_graph_path included after vertex
...
class HashGraph {
  ...
  public:
    ...
    HashGraphVertex *InsertVertex(const HashGraphVertex &vertex) { return &vertex_table_.find_or_insert(vertex); }
    ...
    void BackupEdges() {
        BackupEdgesFunc func;
        vertex_table_.for_each(func);
    }

    void RestoreAndMergeEdges() {
        RestoreAndMergeEdgesFunc func;
        vertex_table_.for_each(func);
    }

    void AddAllEdges() {
        AddAllEdgesFunc func;
        vertex_table_.for_each(func);
        RefreshEdges();
    }
    void ClearEdges() {
    ...
    void SetCountCap(int cap) {
        SetCountCapFunc func(cap);
        vertex_table_.for_each(func);
    }

    void Refresh(int min_count = 0) {
        RefreshVertices(min_count);
        RefreshEdges();
    }
    int64_t RefreshVertices(int min_count = 0) {
        RefreshVerticesFunc func(min_count);
        return vertex_table_.remove_if(func);
    }
    void RefreshEdges() {
        RefreshEdgesFunc func(this);
        vertex_table_.for_each(func);
        num_edges_ = func.num_edges();
    }
  ...
  private:
    ...
    class ClearCountFunc {
      public:
        ...
        void operator()(HashGraphVertex &vertex) { vertex.count() = 0; }
    };
    ...
    class BubbleFunc {
      public:
        BubbleFunc(HashGraph *hash_graph) {
            hash_graph_ = hash_graph;
            omp_init_lock(&bubble_lock_);
        }
        ~BubbleFunc() { omp_destroy_lock(&bubble_lock_); }
    ...
    class AssembleFunc {
      public:
        AssembleFunc(HashGraph *hash_graph) : hash_graph_(hash_graph) { omp_init_lock(&contig_lock_); }
        ~AssembleFunc() { omp_destroy_lock(&contig_lock_); }
...
inline std::istream &operator>>(std::istream &is, HashGraph &hash_graph) { return is >> hash_graph.vertex_table_; }

inline std::ostream &operator<<(std::ostream &os, HashGraph &hash_graph) {
    os << hash_graph.vertex_table_;
