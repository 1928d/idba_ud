...
double ContigGraph::Threshold(double k, double mean, double sd, double p_false) {
    ...
    while (sum < p_false && x < mean) {
        double y = 1 / sqrt(2 * Pi * sd * sd) * exp(-(x - mean) * (x - mean) / (2 * sd * sd)) * 4 * k *
                   p_table[1][int(x)] * step;
        sum += y;
        x += step;
    }
            ...
            if (current.out_edges().size() > 1) {
                ...
                for (unsigned j = 0; j < neighbors.size(); ++j) {
                    ...
                    for (unsigned k = j + 1; k < neighbors.size(); ++k) {
                        if (!neighbors[k].status().IsDead() && neighbors[j].in_edges() == neighbors[k].in_edges() &&
...
#pragma omp parallel for
    for (int64_t i = 0; i < (int64_t)vertices_.size(); ++i) {
        for (int strand = 0; strand < 2; ++strand) {
            ...
            GetNeighbors(current, neighbors);
            for (unsigned j = 0; j < neighbors.size(); ++j) {
                if (neighbors[j].in_edges().size() == 1 && neighbors[j].out_edges().size() == 0 &&
...
double ContigGraph::IterateLocalCoverage(int min_length, double ratio, double min_cover, double max_cover,
                                         double factor) {
...
double ContigGraph::IterateComponentCoverage(int min_length, double ratio, double min_cover, double max_cover,
                                             double factor, int max_component_size) {
    in_kmer_count_table_.reserve(vertices_.size());
...
double ContigGraph::IterateComponentCoverage2(int min_length, double ratio, double min_cover, double max_cover,
                                              double factor, int max_component_size) {
    in_kmer_count_table_.reserve(vertices_.size());
...
#pragma omp parallel for schedule(static, 1)
    for (int64_t i = 0; i < (int64_t)vertices_.size(); ++i) {
        ...
        if (current.contig_size() < min_length + kmer_size_ - 1 &&
            ...
            if (min_cover < ratio * mean ||
                ((int)components[id].size() > max_component_size && min_cover < average_coverage[id]))
                is_changed = true;
            else
...
double ContigGraph::LocalCoverageSingle(ContigGraphVertexAdaptor current, int region_length, double &total_count,
                                        int &total_kmer) {
    map<int, int> visited;
...
bool ContigGraph::IsConverged(ContigGraphVertexAdaptor current) {
    ...
    while (!qu.empty()) {
        ...
        // cout << (reachable[search_node.node] == current.out_edges()) << " " << reachable[search_node.node] << " " <<
        // (int)current.out_edges() << endl;
        ...
        for (int x = 0; x < 4; ++x) {
            if (search_node.node.out_edges()[x]) {
                ...
                new_search_node.distance =
                    search_node.distance + (int)search_node.node.contig_size() - (int)kmer_size_ + 1;
...
void ContigGraph::GetComponents(deque<deque<ContigGraphVertexAdaptor>> &components, deque<string> &component_strings) {
    ...
    for (unsigned i = 0; i < vertices().size(); ++i) {
        ...
        for (int index = 0; index < (int)qu.size(); ++index) {
            ...
            for (int strand = 0; strand < 2; ++strand) {
                // for (connection_list_iterator p = connections()[current].begin(); p != connections()[current].end();
                // ++p)
                for (int x = 0; x < 4; ++x) {
                    if (current.out_edges()[x]) {
                        ContigGraphVertexAdaptor next = GetNeighbor(current, x);
                        ...
                        if (strand == 0) {
                            ss << current.id() << "_" << current.is_reverse() << "_" << current.contig_size() << "_"
                               << current.kmer_count() << " " << next.id() << "_" << next.is_reverse() << "_"
                               << next.contig_size() << "_" << next.kmer_count() << endl;

                            if (!next.status().IsUsed())
                                qu.push_back(next);
                        } else {
                            ss << next.id() << "_" << next.is_reverse() << "_" << next.contig_size() << "_"
                               << next.kmer_count() << " " << current.id() << "_" << current.is_reverse() << "_"
                               << current.contig_size() << "_" << current.kmer_count() << endl;
...
double ContigGraph::FindSimilarPath(ContigGraphVertexAdaptor target, ContigGraphVertexAdaptor start) {
    if (start.status().IsDead() || target.begin_kmer(kmer_size_ - 1) != start.begin_kmer(kmer_size_ - 1) ||
