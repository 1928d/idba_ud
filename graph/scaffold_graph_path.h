...
class ScaffoldGraphPath {
  public:
    ScaffoldGraphPath() {}
    ScaffoldGraphPath(const ScaffoldGraphPath &path) : vertices_(path.vertices_), distances_(path.distances_) {}
    ...
    ScaffoldGraphVertexAdaptor &operator[](uint32_t index) { return vertices_[index]; }
    const ScaffoldGraphVertexAdaptor &operator[](uint32_t index) const { return vertices_[index]; }
