...
class ScaffoldGraphEdge {
  public:
    ...
    ScaffoldGraphEdge(int level, ScaffoldGraphVertexAdaptor from, ScaffoldGraphVertexAdaptor to, int d) {
        from_ = from;
        to_ = to;
        values_.push_back(d);
        level_ = level;
...
class ScaffoldGraphEdgeAdaptor {
  public:
    ...
    ScaffoldGraphEdgeAdaptor(const ScaffoldGraphEdgeAdaptor &x) { edge_ = x.edge_, is_reverse_ = x.is_reverse_; }

    ScaffoldGraphVertexAdaptor from() const { return !is_reverse_ ? edge_->from() : edge_->to().ReverseComplement(); }

    ScaffoldGraphVertexAdaptor to() const { return !is_reverse_ ? edge_->to() : edge_->from().ReverseComplement(); }
...
class ScaffoldGraph {
  public:
    explicit ScaffoldGraph(uint32_t kmer_size = 0) : contig_graph_(kmer_size), min_pairs_(5) { Initialize(); }
    ...
    void AddPair(int level, int from, int to, int d) { pairs_.push_back(ScaffoldGraphPair(level, from, to, d)); }
    ...
    std::deque<ScaffoldGraphEdgeAdaptor> GetEdges(int level, ScaffoldGraphVertexAdaptor current) {
        ...
        for (unsigned i = 0; i < all_edges.size(); ++i) {
            if (all_edges[i].level() == level && !all_edges[i].status().IsDead())
    ...
    void clear() { contig_graph_.clear(); }
