 ...
 * @tparam HashFunc
 */
template <typename Value, typename HashFunc = Hash<Value>, typename EqualKey = std::equal_to<Value>> class HashSet {
  public:
    ...
    HashSet(const hash_set_type &hash_set) : hash_table_(hash_set.hash_table_) {}
    ...
    iterator find(const value_type &value) { return hash_table_.find(value); }

    const_iterator find(const value_type &value) const { return hash_table_.find(value); }

    size_type remove(const value_type &value) { return hash_table_.remove(value); }
    ...
    void swap(hash_set_type &hash_set) {
        if (this != &hash_set)
            hash_table_.swap(hash_set.hash_table_);
    }
...
inline void swap(HashSet<Value, HashFunc, EqualKey> &x, HashSet<Value, HashFunc, EqualKey> &y) {
    x.swap(y);
}
} // namespace std
