...
template <typename T> struct ManagedListNode {
    ManagedListNode<T> *next;
    T value;
};

template <typename T> class ManagedList;

template <typename T> class ManagedListIterator {
  public:
    ...
    ManagedListIterator(node_type *current = NULL) : current_(current) {}
    ManagedListIterator(const iterator &iter) : current_(iter.current_) {}
...
template <typename T> class ManagedListConstIterator {
  public:
    typedef T value_type;
    typedef value_type *pointer;
    typedef const value_type *const_pointer;
    ...
    ManagedListConstIterator(const node_type *current = NULL) : current_(current) {}
    ManagedListConstIterator(const const_iterator &iter) : current_(iter.current_) {}
...
template <typename T> class ManagedList {
  public:
    ...
    ManagedList() : size_(0), head_(NULL), pool_(NULL) {}
    explicit ManagedList(node_pool_type &pool) : size_(0), head_(NULL), pool_(&pool) {}
    ManagedList(const list_type &list) : size_(0), head_(NULL), pool_(list.pool_) { assign(list); }

    ~ManagedList() { clear(); }

    const list_type &operator=(const list_type &list) { return assign(list); }
...
namespace std {
template <typename T> inline void swap(ManagedList<T> &x, ManagedList<T> &y) { x.swap(y); }
} // namespace std
