 ...
 * @tparam HashFunc
 */
template <typename Key, typename Value, typename HashFunc = Hash<Key>, typename EqualKey = std::equal_to<Key>>
class HashMap {
  public:
    typedef HashTable<std::pair<Key, Value>, Key, HashFunc, Select1st<std::pair<Key, Value>>, EqualKey> hash_table_type;
    ...
    HashMap(const hash_map_type &hash_map) : hash_table_(hash_map.hash_table_) {}
    ...
    std::pair<iterator, bool> insert(const value_type &value) { return hash_table_.insert_unique(value); }

    iterator find(const key_type &key) { return hash_table_.find(key); }

    const_iterator find(const value_type &value) const { return hash_table_.find(value); }

    data_type &operator[](const key_type &key) {
        return hash_table_.find_or_insert(value_type(key, data_type())).second;
    }

    size_type remove(const key_type &key) { return hash_table_.remove(key); }

    template <typename Predicator> size_type remove_if(Predicator &predicator) {
        return hash_table_.remove_if(predicator);
    }

    template <typename UnaryProc> UnaryProc &for_each(UnaryProc &op) { return hash_table_.for_each(op); }

    const hash_func_type &hash_func() const { return hash_table_.hash_func(); }
    const key_equal_func_type &key_equal_func() const { return hash_table_.key_equal_func(); }

    void reserve(size_type capacity) { hash_table_.reserve(capacity); }

    void swap(hash_map_type &hash_map) {
        if (this != &hash_map)
            hash_table_.swap(hash_map.hash_table_);
    }
...
inline void swap(HashMap<Key, Value, HashFunc, EqualKey> &x, HashMap<Key, Value, HashFunc, EqualKey> &y) {
    x.swap(y);
}
} // namespace std
