...
template <typename Value, typename Key, typename HashFunc, typename ExtractKey, typename EqualKey> class HashTable;
...
class HashTableIterator {
  public:
    ...
    HashTableIterator(const iterator &iter) : owner_(iter.owner_), current_(iter.current_) {}
...
class HashTableConstIterator {
  public:
    ...
    HashTableConstIterator(const hash_table_type *owner = NULL, const node_type *current = NULL)
        : owner_(owner), current_(current) {}
    HashTableConstIterator(const const_iterator &iter) : owner_(iter.owner_), current_(iter.current_) {}
...
#pragma omp atomic
    ...
    template <typename Predicator> size_type remove_if(const Predicator &predicator) {
        uint64_t num_removed_nodes = 0;
#pragma omp parallel for
...
#pragma omp atomic
    ...
    template <typename UnaryProc> UnaryProc &for_each(UnaryProc &op) {
#pragma omp parallel for
    ...
    template <typename UnaryProc> UnaryProc &for_each(UnaryProc &op) const {
#pragma omp parallel for
        for (int64_t i = 0; i < (int64_t)buckets_.size(); ++i) {
            for (node_type *node = buckets_[i]; node; node = node->next)
                op(node->value);
...
#pragma omp parallel for
  ...
  private:
    void lock_bucket(uint64_t hash_value) { omp_set_lock(&bucket_locks_[hash_value & (kNumBucketLocks - 1)]); }
    void unlock_bucket(uint64_t hash_value) { omp_unset_lock(&bucket_locks_[hash_value & (kNumBucketLocks - 1)]); }
...
template <typename Value, typename Key, typename HashFunc, typename ExtractKey, typename EqualKey>
std::istream &operator>>(std::istream &is, HashTable<Value, Key, HashFunc, ExtractKey, EqualKey> &hash_table) {
    hash_table.clear();
    Value value;
...
template <typename Value, typename Key, typename HashFunc, typename ExtractKey, typename EqualKey>
std::ostream &operator<<(std::ostream &os, HashTable<Value, Key, HashFunc, ExtractKey, EqualKey> &hash_table) {
