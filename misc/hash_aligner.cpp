...
bool Compare(const HashAlignerRecord &x, const HashAlignerRecord &y) { return x.match_length > y.match_length; }
...
int HashAligner::AlignReadLocal(const Sequence &seq, deque<HashAlignerRecord> &records, int min_match, int max_mismatch,
                                int max_records) {
    vector<uint64_t> words;
    Convert(seq, words);
        ...
        for (position_iterator iter = position_list.begin(); iter != position_list.end(); ++iter) {
            ...
            // if (from >= 0 && to <= sequences_[id].size())
            {
                ...
                if (table.insert(value).second) {
                    ...
                    record.query_to =
                        (to <= sequences_[id].size() ? seq.size() : seq.size() - (to - sequences_[id].size()));
                    record.query_length = seq.size();
...
int HashAligner::AlignSequence(const Sequence &seq, deque<HashAlignerRecord> &records, int min_match, double similar,
                               int max_records) {
    vector<uint64_t> words;
    Convert(seq, words);
...
void HashAligner::Match(const vector<uint64_t> &words, HashAlignerRecord &record, int max_mismatch) {
    vector<uint64_t> &ref_words =
        (!record.is_reverse ? sequence_words[record.ref_id] : reverse_sequence_words[record.ref_id]);
