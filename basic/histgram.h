 ...
 * @tparam T
 */
template <class T> class Histgram {
  public:
    typedef T value_type;

    Histgram() : size_(0) { omp_init_lock(&lock_); }
    Histgram(const Histgram<T> &hist) : map_(hist.map_), size_(hist.size_) { omp_init_lock(&lock_); }
    ...
    double up_mean(double p) const {
        ...
        unsigned sum = 0;
        double total = 0;
        for (typename std::map<value_type, unsigned>::const_reverse_iterator iter = map_.rbegin(); iter != map_.rend();
             ++iter) {
            sum += iter->second;
    ...
    unsigned Trim(double fraction) {
        ...
        sum = 0;
        for (typename std::map<value_type, unsigned>::reverse_iterator iter = map_.rbegin(); iter != map_.rend();
             ++iter) {
            if (sum + iter->second <= trim_size) {
                sum += iter->second;
                trim_values.push_back(iter->first);
...
namespace std {
template <typename T> inline void swap(Histgram<T> &histgram1, Histgram<T> &histgram2) { histgram1.swap(histgram2); }
} // namespace std
