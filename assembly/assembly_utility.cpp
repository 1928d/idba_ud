...
uint64_t WriteContig(FastaWriter &writer, const std::deque<Sequence> &contigs,
    ...
    for (unsigned i = 0; i < contigs.size(); ++i) {
        if ((int64_t)contigs[i].size() >= min_contig) {
            ++num_contigs;
            writer.Write(contigs[i], FormatString("%s_%d length_%d read_count_%d", prefix.c_str(), i,
                                                  (int)contigs[i].size(), (int)contig_infos[i].kmer_count()));
        }
    }
    return num_contigs;
}
...
void EstimateDistance(const string &align_file, double &mean, double &sd) {
    ...
    while (ReadHashAlignerRecords(falign, records1) && ReadHashAlignerRecords(falign, records2)) {
...
void BuildKmerFile(AssemblyInfo &assembly_info, int kmer_size, int min_count, int prefix_length,
                   const std::string &kmer_file) {
...
void IterateHashGraph(AssemblyInfo &assembly_info, int new_kmer_size, int min_support, HashGraph &hash_graph,
...
#pragma omp parallel for schedule(static, 1)
    for (int64_t i = 0; i < (int64_t)contigs.size(); ++i) {
        ...
        seq.Assign(contigs[i], max(0, (int)contigs[i].size() - new_kmer_size),
                   min(new_kmer_size, (int)contigs[i].size()));
        old_hash_graph.InsertKmers(seq);
    }
...
int64_t AlignReads(AssemblyInfo &assembly_info, HashAligner &hash_aligner, double similar,
...
void CorrectReads(AssemblyInfo &assembly_info, std::deque<Sequence> &contigs, std::deque<ContigInfo> &contig_infos,
                  const std::string &align_file, int max_mismatch, int min_match) {
...
#pragma omp parallel for
        for (int64_t i = 0; i < size; ++i) {
            ...
            if (record.match_length != 0)
            // if (record.match_length != 0 && record.query_length -
            // record.match_length <= max_mismatch)
            {
                //#pragma omp atomic
        ...
        for (int64_t i = 0; i < size; ++i) {
            ...
            // record.match_length <= max_mismatch)
            {
                ...
                if (match_count >= (int)seq.size() - max_mismatch && is_all_confirmed) {
                    ...
                    flags[i] = false;
                    for (unsigned j = 0; j < seq.size(); ++j) {
                        if (scores[record.ref_from + j].best != -1 && seq[j] != contig[record.ref_from + j]) {
...
#pragma omp parallel for
    ...
    cout << "confirmed bases: " << conformed_bases << " correct reads: " << corrected_reads
         << " bases: " << corrected_bases << endl;
