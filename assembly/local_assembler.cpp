...
void *LocalAssembler::LocalAssembleThread(void *p) {
    ...
    for (int64_t i = task.id; i < (int64_t)contigs.size(); i += num_threads) {
        ...
        for (int strand = 0; strand < 2; ++strand) {
            ...
            task.local_contigs.insert(task.local_contigs.end(), one_round_local_contigs.begin(),
                                      one_round_local_contigs.end());
...
void LocalAssembler::LocalAssemble(Sequence &contig, std::deque<uint64_t> &local_reads,
...
void *LocalAssemblerWithReference::LocalAssembleThread(void *p) {
    ...
    for (unsigned x = 0; x < read_tables.size(); ++x) {
        for (unsigned y = task.id; y < read_tables[x].size(); y += num_threads) {
            ...
            // task.local_contigs.insert(task.local_contigs.end(), local_assembly_info.long_reads.begin(),
            // local_assembly_info.long_reads.end());
