...
class Sequence {
  public:
    ...
    explicit Sequence(const ShortSequence &short_seq) { Assign(short_seq); }
    explicit Sequence(const Kmer &kmer) { Assign(kmer); }
    ...
    const Sequence &operator=(const CompactSequence &compact_seq) {
        Assign(compact_seq);
        return *this;
    }
    const Sequence &operator=(const ShortSequence &short_seq) {
        Assign(short_seq);
        return *this;
    }
    ...
    const Sequence &Assign(const Sequence &seq, int offset = 0, size_t length = std::string::npos) {
        if (&seq != this)
            bases_.assign(seq.bases_, offset, length);
        return *this;
    }
...
namespace std {
template <> inline void swap(Sequence &seq1, Sequence &seq2) { seq1.swap(seq2); }
} // namespace std
