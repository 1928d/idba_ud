...
const Sequence &Sequence::ReverseComplement() {
    ...
    for (unsigned i = 0; i < bases_.size(); ++i) {
        switch (bases_[i]) {
        case 'a':
        case 'A': bases_[i] = 'T'; break;

        case 'c':
        case 'C': bases_[i] = 'G'; break;

        case 'g':
        case 'G': bases_[i] = 'C'; break;

        case 't':
        case 'T': bases_[i] = 'A'; break;

        case 0:
        case 1:
        case 2:
        case 3: bases_[i] = 3 - bases_[i]; break;

        case 'N':
        case 4: break;

        default: throw logic_error("reverse complement error: unkown character.");
        }
    }
...
void Sequence::Encode() {
    for (unsigned i = 0; i < bases_.size(); ++i) {
        switch (bases_[i]) {
        case 'a':
        case 'A': bases_[i] = 0; break;

        case 'c':
        case 'C': bases_[i] = 1; break;

        case 'g':
        case 'G': bases_[i] = 2; break;

        case 't':
        case 'T': bases_[i] = 3; break;

        case 'n':
        case 'N': bases_[i] = 4; break;
...
void Sequence::Decode() {
    for (unsigned i = 0; i < bases_.size(); ++i) {
        switch (bases_[i]) {
        case 0: bases_[i] = 'A'; break;

        case 1: bases_[i] = 'C'; break;

        case 2: bases_[i] = 'G'; break;

        case 3: bases_[i] = 'T'; break;

        case 4: bases_[i] = 'N'; break;

        default: throw logic_error("decode error: unkown character.");
        }
    }
}
