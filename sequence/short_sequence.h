...
class ShortSequence {
  public:
    ...
    ShortSequence(const ShortSequence &short_seq) { Assign(short_seq); }
    ...
    explicit ShortSequence(const Kmer &kmer) { Assign(kmer); }

    const ShortSequence &operator=(const ShortSequence &short_seq) {
        Assign(short_seq);
        return *this;
    }
    const ShortSequence &operator=(const Sequence &seq) {
        Assign(seq);
        return *this;
    }
    const ShortSequence &operator=(const CompactSequence &compact_seq) {
        Assign(compact_seq);
        return *this;
    }
    const ShortSequence &operator=(const Kmer &kmer) {
        Assign(kmer);
        return *this;
    }

    const ShortSequence &Assign(const ShortSequence &short_seq) {
        std::memcpy(data_, short_seq.data_, kNumBytes);
        return *this;
    }
    ...
    uint32_t size() const { return data_[kNumBytes - 2] + (data_[kNumBytes - 1] << 8); }
    void resize(uint32_t new_size) {
        data_[kNumBytes - 2] = (new_size & ((1 << 8) - 1));
        data_[kNumBytes - 1] = (new_size >> 8);
    }
    bool empty() const { return data_[kNumBytes - 2] == 0 && data_[kNumBytes - 1] == 0; }
    ...
    bool operator!=(const ShortSequence &seq) const { return !(*this == seq); }
...
namespace std {
template <> inline void swap(ShortSequence &short_seq1, ShortSequence &short_seq2) { short_seq1.swap(short_seq2); }
} // namespace std
