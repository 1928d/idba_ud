...
class CompactSequence {
  public:
    ...
    explicit CompactSequence(const ShortSequence &short_seq) { Assign(short_seq); }
    explicit CompactSequence(const Kmer &kmer) { Assign(kmer); }

    const CompactSequence &operator=(const CompactSequence &compact_seq) {
        Assign(compact_seq);
        return *this;
    }
    const CompactSequence &operator=(const Sequence &seq) {
        Assign(seq);
        return *this;
    }
    const CompactSequence &operator=(const ShortSequence &short_seq) {
        Assign(short_seq);
        return *this;
    }
    const CompactSequence &operator=(const Kmer &kmer) {
        Assign(kmer);
        return *this;
    }
    ...
    const CompactSequence &operator+=(uint8_t ch) {
        Append(ch);
        return *this;
    }
    ...
    const CompactSequence &Assign(const CompactSequence &compact_seq, int offset = 0,
                                  size_t length = std::string::npos) {
        if (&compact_seq != this) {
            resize(0);
            Append(compact_seq, offset, length);
        }
        return *this;
    }
    const CompactSequence &Assign(const Sequence &seq, int offset = 0, size_t length = std::string::npos) {
        resize(0);
        Append(seq, offset, length);
        return *this;
    }
    const CompactSequence &Assign(const ShortSequence &short_seq);
    const CompactSequence &Assign(const Kmer &kmer);

    const CompactSequence &Append(const CompactSequence &compact_seq, int offset = 0,
                                  size_t length = std::string::npos);
    ...
    uint8_t operator[](uint32_t index) const { return (data_[index >> 2] >> ((index & 3) << 1)) & 3; }
    uint8_t get_base(uint32_t index) const { return (data_[index >> 2] >> ((index & 3) << 1)) & 3; }
    ...
    void swap(CompactSequence &compact_seq) {
        if (this != &compact_seq)
            data_.swap(compact_seq.data_);
    }

    uint32_t size() const { return ((data_.size() - 1) << 2) - data_[data_.size() - 1]; }
    void resize(int new_size) {
        data_.resize((new_size + 7) >> 2);
        data_[data_.size() - 1] = ((data_.size() - 1) << 2) - new_size;
    }
